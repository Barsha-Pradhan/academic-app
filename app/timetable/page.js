...
export default function TimetablePage() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      setUser(userData)
      const { data: ttData } = await supabase.from('timetable').select('*, courses(name)').order('day')
      setTimetable(ttData || [])
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        {showAdd && (
          <div className="bg-white p-5 rounded-xl shadow mb-6">
            <h3 className="font-semibold mb-3 text-gray-800">New Timetable Slot</h3>
            <select className="w-full border p-3 rounded-lg mb-3 text-sm text-gray-800" value={courseId} onChange={e => setCourseId(e.target.value)}>
              {courses.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <select className="w-full border p-3 rounded-lg mb-3 text-sm text-gray-800" value={day} onChange={e => setDay(e.target.value)}>
              {days.map(d => <option key={d} value={d}>{d}</option>)}
            </select>
            <input type="time" className="w-full border p-3 rounded-lg mb-3 text-sm text-gray-800" value={time} onChange={e => setTime(e.target.value)} />
            <input type="text" placeholder="Room (e.g. Room 101)" className="w-full border p-3 rounded-lg mb-3 text-sm text-gray-800 placeholder-gray-400" value={room} onChange={e => setRoom(e.target.value)} />
            ...
            <div key={d} className="mb-4">
              ...
              {slots.map(slot => (
                <div key={slot.id} className="bg-white p-4 rounded-xl shadow mb-2 flex justify-between items-center">
                  <div>
                    <p className="font-medium text-gray-800">{slot.courses?.name}</p>
